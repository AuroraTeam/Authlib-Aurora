...
public class YggdrasilSocialInteractionsService implements SocialInteractionsService {
   ...
   private Set<UUID> fetchBlockList() {
      try {
         BlockListResponse response = (BlockListResponse)this.authenticationService.makeRequest(this.routeBlocklist, (Object)null, BlockListResponse.class, "Bearer " + this.accessToken);
   ...
   private void checkPrivileges() throws AuthenticationException {
      PrivilegesResponse response = (PrivilegesResponse)this.authenticationService.makeRequest(this.routePrivileges, (Object)null, PrivilegesResponse.class, "Bearer " + this.accessToken);
      if (response == null) {
